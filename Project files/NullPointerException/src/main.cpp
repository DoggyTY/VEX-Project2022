...
int Connections[26][2] = { // 0 -> 367, but this ones worse... than the one above
                  //First 8 top
                  {0,1},{0,2},{0,16},
                  {1,3},{1,17},
                  {2,4},{2,18},
...
void gps() {
  ...
  double timebetweengps = timer.timer();
  double LocalXrpm = ((Xrotation.velocity(rpm) / 60) / 1000) * timebetweengps; // divide by 60 to get rps, which would be useful later... also need to convert it to a smaller number, like 0.01 millisecond because brain processes things at 1.3 trillion inputs a seconds
  double LocalYrpm = ((Yrotation.velocity(rpm)) / 1000) * timebetweengps;
  if (Xrpm == 0 and Yrpm == 0){
    return NULL;
  }
  double Rotation = heading.heading(deg);
  Xrpm = (sin(Rotation)*Lo  calYrpm) + (sin(Rotation+90)*LocalXrpm);
  ...
  Controller1.Screen.set Cursor(0,1);
...
void ScreenAnime() {
  ...
  for (auto& combo: Connections) {
    brain.lcd.DrawLine(ProjectedXY[combo[0]][0], ProjectedXY[combo[0]][1], ProjectedXY[combo[1]][0], ProjectedXY[combo[1]][1])
  }
}
